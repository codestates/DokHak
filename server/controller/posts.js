const { Post, User, Stack, post_stack } = require('../models');
...
module.exports = {
  ...
  getPostsByStackId: async (req, res) => {
    try {
      ...
      let postid = await db.sequelize.models.post_stack.findAll({
        // 받은 params.id에 포함된 post.id를 찾는다.
        attributes: ['PostId'],
        where: {
          StackId: param,
        },
  ...
  updatePost: async (req, res) => {
    try {
      ...
      await db.sequelize.models.post_stack.destroy({
        // stackId를 삭제하고
        where: {
          PostId: param,
        },
